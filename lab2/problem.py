...
class Experiment:
    ...
    def run(self) -> None:
        self.penalties = []
        for _ in tqdm(range(self.number_of_episodes + 1)):
            episode_penalty = self._episode()
            self.penalties.append(episode_penalty)
            self.current_episode_no += 1

    def show_off_your_skills(self):
        self.penalties = []
        self.force_draw = True
        old_experiment_rate = self.driver.experiment_rate
        self.driver.experiment_rate = 0
        for _ in tqdm(range(self.show_off_runs)):
            episode_penalty = self._episode()
            self.penalties.append(episode_penalty)
            self.current_episode_no += 1
        self.driver.experiment_rate = old_experiment_rate
    ...
    def _draw_episode(self, positions: list[Position]) -> None:
        if self.force_draw:
            ...
            utils.draw_penalties_plot(self.penalties, 1, self.current_episode_no)
        elif self.current_episode_no % DRAWING_FREQUENCY == 0:
            utils.draw_episode(self.environment.corner.image, positions, self.current_episode_no)
